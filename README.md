...
2. [Обработка для поиска полей с недостающим атрибутом SET STORAGE PLAIN для PostgreSQL](#epf2)
3. [Файл настроек технологического журнала для логирования запросов при реструктуризации v2](#logcfg)
4. [Ошибки при реструктуризации v2 (номер ошибки, номера релизов с исправлением)](#issues)
5. [Ветка на партнерском форуме 1С про неоптимальное удаление реквизита документа для PostgreSQL](#issue_delete)
6. [Небольшое замечание про v2 и секционированные таблицы на MS-SQL](#partitions)
...
## Обработка для поиска полей с недостающим атрибутом SET STORAGE PLAIN для PostgreSQL <a name="epf2"></a>
Обработка работает на сервере 1С под Windows используя COM-объект ADODB.

Обработка проверяет, что база в которой запущена обработка и база подключенная через ADODB одна и та же.

Обработка для PostgreSQL отбирает поля, для которых скорее всего требуется установить SET STORAGE PLAIN.

Условия, по которым устанавливается признак SET STORAGE PLAIN установлены путем тестирования 8.3.23.1865,
поэтому для других версий платформы лучше на тестовой базе перепроверять для каких полей 1С ставит признак.
Также не все метаданные были использованы при тестах.

По кнопке "Скрипт" генерируется скрипт установки признака SET STORAGE PLAIN.

В примере на скриншоте удален типовой индекс, а вместо типового добавлен вручную индекс с полем include.
![Разница_SET_STORAGE_PLAIN.epf](./Разница_SET_STORAGE_PLAIN.epf)
![Обработка](./img/Обработка2.png)
